...
>
> Version 1.0 · 15.08.2026 · Document owner: Lead
...
## 🔴 Week 1 task — name verification (Lead, before any design work)
...
>
> The dark theme is dropped. Lead's decision: the buyer is a restaurant owner as
...
Lead's original framing was: *"if we were certified and could issue a confirmation
...
### 🔴 Step 1 is Lead's day-1 task. We do not start selling without it
...
Our equivalent: runs against **Attacks's test bots**, not unit tests.
...
|---|---|---|---|---|
| 🎯 | **Lead** | coordination, design, direction, positioning, name verification, the 24-site hypothesis | the "X of 24" number + design concept + this document | backend code |
| ⚙️ | **Backend** | core engine: scanning, API, database, ownership verification | working end-to-end scan + verification | copy, legal wording |
| 🧪 | **Attacks** | target lab: vulnerable and hardened bots, calibration set | 3 bots + a 30-item calibration set | production code |
| ⚖️ | **GRC** | GRC: regulation, legal hooks, CTA copy, disclaimers | legal map + 5 verified hooks | code |
...
5. **GRC writes all legal wording.** Not Lead, not Claude Code. Any text containing "Gesetz", "Pflicht", "Art." or "§" goes through him.
6. **Lead makes design decisions.** Discussion before, no appeals after.

### Role briefs

Each person reads their file in `docs/`:

- `docs/IMPLEMENTATION-PLAN.md` — technical spec
- `docs/TARGET-LAB.md` — target lab
- `docs/GRC-BRIEF.md` — legal research
- this file — everyone
...
|---|---|
| The word "zertifiziert" slips into copy | GRC proofreads **every** text before publication |
...
| Judge produces false positives | Attacks's calibration set + three confidence levels |
...
├── tools/art50check.py      hypothesis check, day 1 (Lead)
└── docs/
