...
(`TARGET-LAB.md:145`), and reading it from the bot's own YAML makes it
...
`TARGET-LAB.md:90` shows a third label, `human_label: warn`.
...
- The output block the brief itself asks for (`TARGET-LAB.md:120-129`)
  counts *false positives (pass→fail)* and *false negatives (fail→pass)* — a
  binary comparison.
- `TASK-ATTACKS.md:17` states the deliverable as "30 answers labelled PASS or
  FAIL".
...
(`TARGET-LAB.md:131`) — an invented vulnerability in a paid Prüfbericht
