...
> §11 rule 5: "GRC writes all legal wording").

Per `docs/GRC-BRIEF.md` §3 (D2), the target for this deliverable is
...
`docs/GRC-BRIEF.md` §3; not yet independently re-verified against a
...
`docs/GRC-BRIEF.md` §3 (D2) calls for four more hooks in the same
...
|---|---|---|
| Air Canada — contractual bindingness of chatbot statements | `UNCLEAR` | Real Canadian precedent (BC Civil Resolution Tribunal, Feb 2024) exists and is documented in `PROJECT-STATE.md` and `LEGAL-MAP.md`, but `docs/GRC-BRIEF.md` D3 flags an open question — whether a German/EU equivalent case exists — as unresolved. Publish only with an explicit "Canadian precedent, illustrative only" label until that is resolved. |
...
| AI Act Art. 99 — penalty ceiling | `UNDER REVIEW` | See `LEGAL-MAP.md` row on Art. 99 — which penalty tier applies to an Art. 50 breach specifically is an open question per `docs/GRC-BRIEF.md` D1. |
