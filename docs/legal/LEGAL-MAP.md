...
| **German § 5 UWG (misleading commercial practice)** | Prohibits misleading claims in commercial communication. | Constrains **PromptGuard itself** as a seller, not the customer's chatbot. | N/A (a constraint on our own marketing, not a target-testable property). | Enforced structurally by `docs/legal/FORBIDDEN-WORDS.md` and GRC's sign-off requirement (`PLAYBOOK.md` Part V §11 rule 5). | PromptGuard must not overstate legal risk, uniqueness, or certification status in its own marketing. | `PRIMARY AUTHORITY` | `VERIFIED` |
